...
_The "AI OS" / second-brain pattern, ported from Claude-desktop-host to Delta-host. The five-skill framing is Ben Ang's; the mapping onto Delta is ours._

## Vault structure

OS Setup creates this layout in the user-picked folder. The exact shape comes from Ben's walkthrough; the wording of the `Claude.md` files is paraphrased so each one fits Delta's voice.
...
2. Delta creates the folder structure listed above + writes the `Claude.md` maps.
...
**Why we split it this way:** Ben's wizard does setup + brain-dump in one go because Claude desktop has nothing else for the user to do meanwhile. Delta is a browser — the user is going to walk away mid-wizard. Persisting "I have a vault" separately from "I've answered the 12 sections" lets you come back and pick up later without restarting.
...
**What it does in Delta:** a Delta scheduled task (we already shipped the infra — [`apps/browser/src/main/schedules.ts`](../../browser/src/main/schedules.ts)) with `action: "agent"` and a system prompt that:

- Lists recent Delta conversations (`userData/conversations/*.json.gz`).
...
**What it does in Delta:** same shape — a scheduled task with `action: "agent"` and an audit-style prompt. First PR bundles the prompt + a weekly `every` schedule. A dedicated `audit_vault` agent tool is a future enhancement that returns structured findings rather than free-text.
...
**What it does in Delta:** out of scope while you're solo. When the team comes in, the path is: keep the vault folder under Obsidian Relay sync (Delta doesn't need to know about Relay — it just sees a folder), and we add a `permissions.yaml` per subfolder that Delta's agent honours when writing.
...
**What it does in Delta:** Delta's main process is well-positioned to expose an MCP server pointing at the vault. That's a real PR — needs an MCP transport, auth, and a public endpoint story. We can fork the design when you're ready.

For now the design implication is: keep the vault on disk with no Delta-specific format. Markdown + frontmatter only. That way an MCP server (Delta-built or external) can read it without translation.

## How this fits Delta's existing architecture
...
1. **Where should the default vault live?** Three sane options:
   - `<userData>/second-brain/` — owned by Delta. Wiping Delta wipes the vault. Worst for backup.
   - `~/Documents/Delta Second Brain/` — visible to the user. Better for backup, but conflicts with their existing Documents layout.
